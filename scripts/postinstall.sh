...
# Intranet/offline install: set SICHEK_OFFLINE=1 to skip all OSS access (probe +
# download) and rely solely on pre-staged specs under /var/sichek/config. Online
# installs leave SICHEK_OFFLINE unset and keep the public OSS default below.
if [ "$SICHEK_OFFLINE" = "1" ] || [ "$SICHEK_OFFLINE" = "true" ]; then
    echo "SICHEK_OFFLINE set, configuring offline mode (no OSS spec URL)"
